...
from audio_video_separator import split_video
import data_URI
...
def chat(video_file: str, messages: list[Any], progress: ui.Progress) -> str:
    ...
    with data_URI.as_tempfile(audio_uri) as audio_file:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            *messages,
            {
                "role": "system",
                "content": Path("sys_prompt.txt").read_text(),
            },
        ],
    )
    ...
    response_audio_uri = data_URI.from_bytes(audio.read(), "audio/mpeg")
